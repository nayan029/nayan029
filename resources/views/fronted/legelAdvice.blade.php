                        ...
                        <select class="form-select sa-form-font border-radius-5px" onchange="functest(this.value)" id="cattid">
                            ...
                            <?php foreach ($advicecategory as $data)  { ?>
                                <option  <?php if ($category_name==$data->slug) {
                                    echo "selected";
                                } ?> value="<?php echo $data->slug; ?>">{{{$data->category_name}}} </option>
                            <?php } ?>
                        </select>
                    </div>

                    <div class="col-md-8 d-flex">
                        <form class="form-inline mb-3" method="GET" action="<?php echo URL::to('/') ?>/free-legal-advice/answers">
                                        ...
                                        <div class="sr-card">
                                            <a href="#">
                                                <h5 class="sr-title2 d-flex">
                                                    {{$getquerysdata->question}}</h5>
                                            </a>
                                        </div>
                                        <p class="mb-20"> <?php echo substr($getquerysdata->question_details, 0, 150);?>
                                        </p>
                                        <div class="ml-auto">
                                            <a href="<?php echo URL::to('/') ?>/legal-advice-details/{{$getquerysdata->id}}" class="btn btn-outline-primary">Read More</a>
                                        </div>
                                    </div>
                                </div>
                                <?php }
...
<script>
    function functest(name) {
        ...
        // die();
        $.ajax({
            url: "<?php echo URL::to('/'); ?>/"+sel_val+"-legal-advice/",
            type: "POST",
            data: {
                name: name,
            ...
            success: function(response) {
                ...
                // alert(res)
                window.location.href = "<?php echo URL::to('/'); ?>/"+sel_val+"-legal-advice";
            }
        });
    }
</script>
